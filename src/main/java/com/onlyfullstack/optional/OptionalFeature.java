...
public class OptionalFeature {
    ...
    private static void traditionalApproach(SmartPhone smartPhone) {
        ...
        if (smartPhone.getGraphicsCard().isPresent()) {
            ...
            Optional<GraphicsMemory> graphicsMemoryOptional = Optional.ofNullable(graphicsCard.getGraphicsMemory());
            if (graphicsMemoryOptional.isPresent()) {
                GraphicsMemory graphicsMemory = graphicsMemoryOptional.get();
                Optional<String> dedicatedMemoryOptional = Optional.ofNullable(graphicsMemory.getDedicatedMemory());
                size = dedicatedMemoryOptional.orElse("unknown");
            }
        }
        System.out.println("Size : " + size + ", for Object : " + smartPhone.toString());
    }
